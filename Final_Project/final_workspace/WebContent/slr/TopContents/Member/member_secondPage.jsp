...
function chk() {
	...
	if (document.frm.check2.checked == false) {
...
function checkcheck2(){
	location.href = 'mainSlr.jsp?content=TopContents/Member/member_info.jsp';
}
...
<body>
	...
	<div class="join_wrap">
	<form name='frm' method='post' action='mainSlr.jsp?content=TopContents/Member/member_info.jsp'>
		<div class="page_tt">
			<div>
				<h2>
		...
		<div id="zzz12">
			<div id="zzz1234">
				<img src="./TopContents/Member/img/ggg.png">
			</div>
		</div>

		<div id="z123456">
			<img src="./TopContents/Member/img/hy.gif">
		</div>
		<div id="srl_member_AGD" style="overflow-x:hidden; white-space: pre-line;">
	
01 총칙
제 1조 목적
이 서비스 약관은 회원이 ㈜인비전커뮤니티(이하 "회사")에서 제공하는 인터넷 사이트 SLRCLUB(www.slrclub.com) 서비스(이하 "서비스")를 이용함에 있어 회사와 회원 간의 권리, 의무 및 책임사항 등을 규정함을 목적으로 합니다.
제 2조 용어의 정의
...
아이핀 : 주민등록번호 대체수단으로써 인터넷상에서 신분확인을 위한 식별번호등을 제공하는 대체 신분확인 수단
...
가필수 항목에는 ID, Password, 아이핀(i-PIN), 성명, 닉네임, E-Mail 주소 및 기타 회사가 필요하다고 인정하는 사항이 포함됩니다.
제 7조 이용 신청의 승낙과 제한
...
법률 또는 약관 위반, 기타 이용자의 귀책사유에 의하여 이용 자격이 해지된 적이 있는 이용자가 신청하는 경우, 단 자격 상실 이후 6개월 이상 경과하여 회사의 재가입 승낙을 받은 경우는 예외
...
제 15조 회원 장터
회사는 회원장터 서비스의 이용을 회원 등급별로 제한할 수 있으며 회원장터 서비스 운영에 관한 세부운영지침을 별도로 정하여 시행할 수 있습니다.
회사는 회원의 회원장터 내 활동이 다음 각 호에 해당한다고 판단될 경우 사전 통보 없이 게시물의 게시 중단, 이동, 삭제 등의 조치를 취할 수 있고, 필요하다고 판단되는 경우 해당 회원의 서비스 이용 자격을 제한하거나 정지, 상실시킬 수 있습니다.
타인의 매물 혹은 구매 물품 등을 대리하여 등록하는 행위
영리를 목적으로 회원 장터를 이용하는 행위
현행법상 불법인 물품의 거래를 하는 행위
답글을 통해 다른 회원의 거래를 의도적으로 방해하는 행위. (가격 관련 답글 포함)
타인을 사칭하거나 사기를 목적으로 이용하는 행위
허위 정보를 등록하여 타인에게 피해를 끼치거나 가격 질서를 어지럽히는 등의 행위
기타 관계법령에 위배되거나, 회사에서 정한 게시물 원칙에 어긋나는 경우
회사는 회원이 게재하는 게시물의 내용 또는 거래하는 물품에 대하여 어떠한 보증도 하지 않습니다. 회원이 회원장터 서비스 이용 중에 발생하는 손실이나 기타의 문제는 전적으로 회원 본인의 판단에 따른 책임이며, 회사의 고의가 아닌 한, 회사는 이에 대하여 책임지지 않습니다.
전항의 조건에도 불구하고 회사는 회원이 회원장터 서비스를 이용중에 사기에 의한 피해를 입은 경우 이를 구제하기 위해 수사기관의 자료 요청에 적극적으로 협력하여야 합니다.
회사는 회원장터 서비스를 이용하는 회원간의 분쟁에 따른 요청이나, 실제 피해 사례 및 다른 이용자의 피해 가능성을 근거로 요청을 받은 경우 요청 사유가 소멸되기 전까지 해당하는 회원의 서비스 이용 자격을 임시로 제한할 수 있습니다.
회사는 회원장터내에서 안전거래(매매대금 보호) 서비스를 추가로 제공할 수 있습니다. 이때 관련 서비스를 제공하기 위하여 필요한 최소한의 정보(아이디, 주민번호, 아이핀등 기타 회원의 식별을 위하여 필요한 정보, 계좌번호 등 대금결제에 필요한 정보, 배송에 필요한 성명, 주소, 전화번호 등)를 추가로 요청할 수 있으며 대금 결제 및 배송을 위하여 해당 업체에 이러한 정보를 제공할 수 있습니다.
...
회사는 이용계약을 해지하는 경우 개인정보보호정책에 따라 회원 등록을 말소합니다.
...
부칙
(시행일) 이 약관은 2013년 2월 9일부터 시행됩니다.
2007년 7월 20일부터 시행되던 종래 약관은 본 약관으로 대체됩니다.

</div>
	
	<div id="zz_under">

	<label><input type="checkbox" name="check1"  > 상기 약관을 읽고 이에 동의합니다.</label>
	</div>
	
	<div id="zz22">
		<img src="./TopContents/Member/img/join1_tt_privz.gif" >
	</div>
	
	<div id="srl_member_AGD2" style="overflow-x:hidden; white-space: pre-line;">
	
	
	수집하는 개인정보 항목
회사는 회원가입, 원활한 고객 상담, 서비스 신청 등등을 위해 아래와 같은 개인정보를 수집하고 있습니다.
수집항목 : 이름, 생년월일, 성별, 로그인ID, 자택 전화번호(선택), 자택 주소(선택), 휴대전화번호, 이메일 직업(선택), 회사명(선택), 부서(선택), 회사전화번호(선택), 아이핀 정보(또는 외국인 등록정보), 서비스 이용기록 접속 로그, 쿠키, 접속 IP 정보, 결제기록
개인정보 수집방법 : 홈페이지(회원가입), 서면양식, 전화, 팩스, 배송 요청, 이벤트 응모, 생성정보 수집 툴을 통한 수집, 가입인증정보
개인정보의 수집 및 이용목적
...
상세 수집정보별 수집목적은 다음과 같습니다.
성명,아이핀정보,아이디,비밀번호 : 회원제 서비스 이용에 따른 본인식별
아이디 : 쪽지 보내기, 회원 찾기, 친구 추가를 위한 고유 이용자 특정
아이디, 아이핀정보 : 연령제한 서비스의 제공
아이디, 아이핀정보, 이메일주소 : 불량회원의 부정한 이용 재발 방지, 회원탈퇴 후 1개월간 재가입 방지
이메일주소,전화번호 : 고지사항 전달, 본인 의사확인, 불만처리 등 의사소통 경로의 확보
은행계좌정보,신용카드정보,핸드폰번호,전화번호 : 유료정보이용 및 상품 구매에 대한 결제
주소,전화번호 : 청구서, 경품 및 공동구매 물품배송시 정확한 배송지의 확보
아이핀정보,주소 : 인구통계학적 분석
그 외 선택항목 : 개인맞춤서비스를 제공하기 위한 자료
IP Address, 인식정보, 방문일시 : 불량회원의 부정 이용 방지와 비인가 사용 방지
수집하는 개인정보 항목 및 수집 방법
회사는 이용자들이 회원서비스를 이용하기 위해 회원으로 가입하실 때 서비스 제공을 위한 필수적인 정보들을 온라인상에서 입력 받고 있습니다. 회원 가입시에 받는 필수적인 정보는 아이핀, 이메일주소 등입니다. 또한 양질의 서비스 제공을 위하여 이용자들이 선택적으로 입력할 수 있는 사항으로서 전화번호 등을 입력 받고 있습니다.
또한, SLRCLUB내에서의 설문조사나 이벤트 행사시 통계분석이나 경품제공 등을 위해 선별적으로 개인정보 입력을 요청할 수 있습니다. 그러나, 이용자의 기본적 인권 침해의 우려가 있는 민감한 개인정보(인종 및 민족, 사상 및 신조, 출신지 및 본적지, 정치적 성향 및 범죄기록, 건강상태 및 성생활 등)는 수집하지 않으며 부득이하게 수집해야 할 경우 이용자들의 사전동의를 반드시 구할 것입니다. 그리고, 어떤 경우에라도 입력하신 정보를 이용자들에게 사전에 밝힌 목적 이외에 다른 목적으로는 사용하지 않으며 외부로 유출하지 않습니다.
SLRCLUB에 광고를 게재하는 광고주나 검색 등의 각종 디렉토리 또는 게시판에 링크되어 있는 웹사이트들이 이용자의 개인정보를 수집할 수도 있을 것입니다. SLRCLUB에 링크되어 있는 웹사이트들이 개인정보를 수집하는 행위에 대해서는 본 개인정보 보호정책이 적용되지 않음을 알려 드립니다.
개인정보의 보유 및 이용기간
개인정보 수집 및 이용목적이 달성된 후에는 해당 정보를 지체 없이 파기합니다. 단, 다음의 정보에 대해서는 하기의 이유로 명시한 기간 동안 보존합니다. 관계법령의 규정에 의하여 보존할 필요가 있는 경우 회사는 아래와 같이 관계법령에서 정한 일정한 기간 동안 회원정보를 보관합니다.
보존 항목 : 이름, 생년월일, 성별, 로그인ID, 비밀번호, 자택 전화번호, 자택 주소, 휴대전화번호, 이메일, 직업, 회사명, 부서, 회사 전화번호, 서비스 이용기록, 접속 로그, 접속 IP 정보, 결제기록
...
보존 기간 : 3년
관련법령에 의한 정보보유 사유
상법, 전자상거래 등에서의 소비자보호에 관한 법률 등 관계법령의 규정에 의하여 보존할 필요가 있는 경우 회사는 관계법령에서 정한 일정한 기간 동안 회원정보를 보관합니다. 이 경우 회사는 보관하는 정보를 그 보관의 목적으로만 이용하며 보존기간은 하기와 같습니다.
계약 또는 청약철회 등에 관한 기록
...
보존 이유 : 통신비밀보호법
보존 기간 : 2년
회원가입을 탈퇴한 경우에는 회원 재가입,임의해지등을 반복적으로 행함으로써 SLRCLUB 이 제공하는 서비스의 부정이용 및 이벤트 혜택 등의 이익등을 불-편법적으로 수취하거나 이 과정에서 명의도용 등의 행위가 우려가 있으므로 이러한 행위의 차단 목적으로 회원 탈퇴 후 1개월동안 회원의 성명, 아이핀정보(또는 외국인 등록정보),아이디,비밀번호,이메일 식별정보를 보관합니다.

	</div>
	
	<div id="zz_under2">
	<label><input type="checkbox" name="check2">상기 개인정보 취급방침을 읽고 이에 동의합니다.</label>	
	</div>
	
	<div id="qq_main">
	<div id="qq">
	
		<input type="submit" value="동의" onclick='ggg()'>
		
	</div>
	
	<div id="qq2">
	<a href="mainSlr.jsp?content=TopContents/Member/member_firstPage.jsp">
		<img src="./TopContents/Member/img/qq2.jpg">
		</a>
	</div>
